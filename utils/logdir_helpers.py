...
_LOG_DATE_FORMAT = "%m%d-%H%M"  # month day _ hour minute
_RESTORE_PREFIX = 'RESTORE@'


def create_unique_log_dir(config_path, log_dir_root, line_breaking_chars_pat=r'-', restore_dir=None):
	"""
	unique log dir format: config_name-monthday-hourminute
	"""
	md_hm = datetime.now().strftime("-%m%d-%H%M")  # "0227-1951"
	config_name = path.basename(config_path).split('.')[0]
	
	unique_dir = path.join(log_dir_root, config_name + md_hm)
	
	# assert not path.exists(unique_dir), 'already exist {}'.format(unique_dir)
...
# def iter_ckpt_dirs(log_dir_root, job_ids_str_list):
# 	assert os.path.exists(log_dir_root), "Invalid log dir: {}".format(log_dir_root)
# 	assert len(job_ids_str_list) > 0, "No job_ids!"
#
# 	for job_id in job_ids_str_list:
# 		ckpt_dir_glob = Saver.ckpt_dir_for_log_dir(path.join(log_dir_root, job_id))  # ckpts path
...
# def create_unique_log_dir(config, log_dir_root, line_breaking_chars_pat=r'[-]', restore_dir=None):
#     """
#     log_dir format: config_monthday_hourminute
...
def log_date_from_log_dir(log_dir):
	...
	possible_log_date = '-'.join(possible_log_date.split('-')[-2:])
