...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(train_loader):
        ...
        # rd_loss = torch.mean(train_lambda * distortion + distribution_loss)
        optimizer.zero_grad()
        rd_loss.backward()
...
if __name__ == "__main__":
    ...
    test_loader = DataLoader(dataset=test_dataset, shuffle=False, batch_size=1, pin_memory=True, num_workers=0)
    if args.test:
        testKodak(global_step)
        exit(-1)
    ...
    steps_epoch = global_step // (len(train_dataset) // (batch_size)) # i think have some problem, remove batch_size
    save_model(model, global_step, save_path)
