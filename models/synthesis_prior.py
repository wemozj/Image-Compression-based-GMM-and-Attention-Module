...
class Synthesis_prior_net(nn.Module):
    ...
    def __init__(self, out_channel_N=192, out_channel_M=320):
        ...
        self.deconv3 = nn.ConvTranspose2d(out_channel_N, out_channel_N, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.deconv3.weight.data, (math.sqrt(2 * 1)))
        torch.nn.init.constant_(self.deconv3.bias.data, 0.01)
        self.relu3 = nn.LeakyReLU()
        
        # # channel: N -> M, upsample 1
        # self.deconv1 = nn.ConvTranspose2d(out_channel_N, out_channel_M, 3, stride=1, padding=1)
        # torch.nn.init.xavier_normal_(self.deconv1.weight.data, (math.sqrt(2 * 1)))
        # torch.nn.init.constant_(self.deconv1.bias.data, 0.01)
        # self.relu1 = nn.LeakyReLU()
        #
        # # channel: N -> 2N, upsample: 2
        # self.deconv2 = nn.ConvTranspose2d(out_channel_M, out_channel_M, 3, stride=2, padding=1, output_padding=1)
        # torch.nn.init.xavier_normal_(self.deconv2.weight.data, math.sqrt(2 * 1))
        # torch.nn.init.constant_(self.deconv2.bias.data, 0.01)
        # self.relu2 = nn.LeakyReLU()
        #
        # # channel: N -> 1.5N, upsample: 1
        # self.deconv3 = nn.ConvTranspose2d(out_channel_M, int(out_channel_M*1.5), 3, stride=1, padding=1)
        # torch.nn.init.xavier_normal_(self.deconv2.weight.data, math.sqrt(2 * 1 * 3))
        # torch.nn.init.constant_(self.deconv2.bias.data, 0.01)
        # self.relu3 = nn.LeakyReLU()
        #
        # # channel: 1.5N -> 2N, upsample: 2
        # self.deconv4 = nn.ConvTranspose2d(int(out_channel_M*1.5), int(out_channel_M * 1.5), 3, stride=2, padding=1, output_padding=1)
        ...
        #
        # self.deconv5 = nn.ConvTranspose2d(int(out_channel_M * 1.5), int(out_channel_M * 2), 3, stride=1, padding=1)
        # torch.nn.init.xavier_normal_(self.deconv5.weight.data, (math.sqrt(2 * 1 * (out_channel_M * 2 + out_channel_M * 1.5) / (out_channel_M * 1.5 + out_channel_M * 1.5))))
        # torch.nn.init.constant_(self.deconv3.bias.data, 0.01)
